 ...
 * Egern「网络诊断雷达」Pro
 *
 * 🔄 合并版：A（lylywayr UI框架 + 策略组 + 延迟/出口IP）+ B（xcgtb 精准检测逻辑）+ C（mickeu 双源纯净度）
 *
 * 环境变量：
...
export default async function (ctx) {
  ...
  function latencyLabel(url) {
    ...
    const map = {
      ...
      "qualcomm.cn": "高通", "captive.apple.com": "apple",
  ...
  async function checkNetflixPrecise(policy) {
    ...
    try {
      const fetchTitle = async (url) => {
        try { const r = await ctx.http.get(url, serviceRequestOptions(policy)); return await r.text(); } catch (_) { return ""; }
      };
      const [t1, t2] = await Promise.all([fetchTitle(titles[0]), fetchTitle(titles[1])]);
      const oh1 = /oh no!/i.test(t1 || ""), oh2 = /oh no!/i.test(t2 || "");
      if (!t1 && !t2) return { ok: false, detail: "Cross" };
      if (oh1 && oh2) return { ok: true, detail: "🍿" };
      for (const b of [t1, t2]) {
        if (!b) continue;
        const rm = b.match(/"countryCode"\s*:\s*"?([A-Z]{2})"?/);
        if (rm && rm[1]) return { ok: true, detail: rm[1] };
      }
      return { ok: true, detail: "OK" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkDisneyPrecise(policy) {
    try {
      const res = await ctx.http.get("https://www.disneyplus.com", serviceRequestOptions(policy, { redirect: "manual" }));
      if (!res || res.status === 403) return { ok: false, detail: "Cross" };
      const loc = (res.headers && (res.headers.location || res.headers.Location)) || "";
      return { ok: !loc.includes("unavailable"), detail: loc.includes("unavailable") ? "Cross" : "OK" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkTikTokPrecise(policy) {
    try {
      const getBody = async (url) => {
        try { const r = await ctx.http.get(url, serviceRequestOptions(policy)); return await r.text(); } catch (_) { return ""; }
      };
      let body = await getBody("https://www.tiktok.com/");
      if (body && body.includes("Please wait...")) { body = await getBody("https://www.tiktok.com/explore"); }
      const m = body ? body.match(/"region"\s*:\s*"([A-Z]{2})"/) : null;
      if (m && m[1]) return { ok: true, detail: m[1] };
      return body ? { ok: true, detail: "OK" } : { ok: false, detail: "Cross" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkYouTubePrecise(policy) {
    try {
      const res = await ctx.http.get("https://www.youtube.com/premium", serviceRequestOptions(policy, {
        headers: { ...serviceRequestOptions(policy).headers, "Accept-Language": "en" }
      }));
      const body = await res.text();
      if (!body) return { ok: false, detail: "Cross" };
      if (body.includes("www.google.cn")) return { ok: true, detail: "CN" };
      if (body.includes("Premium is not available") || body.includes("YouTube Premium is not available"))
        return { ok: false, detail: "Cross" };
      const m = body.match(/"contentRegion"\s*:\s*"?([A-Z]{2})"?/);
      if (m && m[1]) return { ok: true, detail: m[1] };
      return (body.includes("ad-free") || body.includes("Ad-free")) ? { ok: true, detail: "OK" } : { ok: false, detail: "Cross" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkChatGPTPrecise(policy) {
    try {
      const webRes = await ctx.http.get("https://chatgpt.com", serviceRequestOptions(policy, { redirect: "manual" }));
      const webOk = !!webRes;
      const iosRes = await ctx.http.get("https://ios.chat.openai.com", serviceRequestOptions(policy));
      const iosBody = iosRes ? await iosRes.text() : "";
      let cfDetails = "";
      try { cfDetails = JSON.parse(iosBody).cf_details || ""; } catch (_) {}
      const appBlocked = !iosBody || iosBody.includes("blocked_why_headline") ||
        iosBody.includes("unsupported_country_region_territory") ||
        cfDetails.includes("(1)") || cfDetails.includes("(2)");
      const appOk = !!iosBody && !appBlocked;
      if (!webOk && !appOk) return { ok: false, detail: "Cross" };
      if (appOk && !webOk) return { ok: true, detail: "APP" };
      const traceRes = await ctx.http.get("https://chatgpt.com/cdn-cgi/trace", serviceRequestOptions(policy));
      const traceBody = await traceRes.text();
      const tm = traceBody ? traceBody.match(/loc=([A-Z]{2})/) : null;
      return { ok: true, detail: tm && tm[1] ? tm[1] : "OK" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkClaudePrecise(policy) {
    try {
      const res = await ctx.http.get("https://claude.ai/login", serviceRequestOptions(policy));
      if (!res) return { ok: false, detail: "Cross" };
      const body = await res.text();
      if (body.includes("App unavailable") || body.includes("certain regions")) return { ok: false, detail: "Cross" };
      if (res.status === 403 && (body.includes("cf-turnstile") || body.includes("Just a moment")))
        return { ok: true, detail: "OK" };
      return (res.status === 200 || res.status === 301 || res.status === 302) ? { ok: true, detail: "OK" } : { ok: false, detail: "Cross" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }

  async function checkGeminiPrecise(policy) {
    try {
      ...
      const res = await ctx.http.post("https://gemini.google.com/_/BardChatUi/data/batchexecute", {
        ...
        body: bodyRaw,
        headers: { ...serviceRequestOptions(policy).headers, "Content-Type": "application/x-www-form-urlencoded" }
      });
      const txt = await res.text();
      if (!txt) return { ok: false, detail: "Cross" };
      let m = txt.match(/"countryCode"\s*:\s*"([A-Z]{2})"/i);
      if (m && m[1]) return { ok: true, detail: m[1].toUpperCase() };
      m = txt.match(/"requestCountry"\s*:\s*\{[^}]*"id"\s*:\s*"([A-Z]{2})"/i);
      if (m && m[1]) return { ok: true, detail: m[1].toUpperCase() };
      m = txt.match(/\[\s*"([A-Z]{2})"\s*,\s*"S/);
      if (m && m[1]) return { ok: true, detail: m[1].toUpperCase() };
      return { ok: true, detail: "OK" };
    } catch (_) { return { ok: false, detail: "Cross" }; }
  }
  ...
  async function testService(id, name, kind, color, url, servicePolicy) {
    ...
    if (!url) {
      const serviceExit = await serviceExitPromise;
      let preciseResult = null;

      if (kind === "netflix") preciseResult = await checkNetflixPrecise(servicePolicy);
      else if (kind === "disney") preciseResult = await checkDisneyPrecise(servicePolicy);
      else if (kind === "tiktok") preciseResult = await checkTikTokPrecise(servicePolicy);
      else if (kind === "youtube") preciseResult = await checkYouTubePrecise(servicePolicy);
      else if (kind === "chatgpt") preciseResult = await checkChatGPTPrecise(servicePolicy);
      else if (kind === "claude") preciseResult = await checkClaudePrecise(servicePolicy);
      else if (kind === "gemini") preciseResult = await checkGeminiPrecise(servicePolicy);

      if (preciseResult) {
        const detail = preciseResult.detail || "";
        return {
          id: id,
          name: name,
          kind: kind,
  ...
  let [
    ...
    Promise.all([
      ...
      testService("disney", "Disney+", "disney", C.disney, "", mediaPolicyMap.disney),
      testService("spotify", "Spotify", "spotify", C.spotify, "https://open.spotify.com/", mediaPolicyMap.spotify),
      testService("tiktok", "TikTok", "tiktok", C.tiktok, "", mediaPolicyMap.tiktok),
      testService("youtube", "YouTube", "youtube", C.youtube, "", mediaPolicyMap.youtube),
      testService("prime", "Prime", "prime", C.prime, "https://www.primevideo.com/", mediaPolicyMap.prime)
    ]),

    Promise.all([
      testService("chatgpt", "ChatGPT", "chatgpt", C.chatgpt, "", aiPolicyMap.chatgpt),
      testService("claude", "Claude", "claude", C.claude, "", aiPolicyMap.claude),
      testService("gemini", "Gemini", "gemini", C.gemini, "", aiPolicyMap.gemini),
          ...
          [
            ...
            col(
              [
                ...
                row(
                  [
                    text(shortISP(exit.isp) + " \u00b7 \u843d\u5730 " + displayIP(exit.ip), 5.15, "medium", C.subtext, {
                      flex: 1,
                      maxLines: 1,
                      minScale: 0.42
                    }),
