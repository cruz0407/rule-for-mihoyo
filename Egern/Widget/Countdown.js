 ...
 * • 自定义配置：支持通过环境变量设置最多 6 个专属纪念日（默认关闭，需 SHOW_EXCLUSIVE_DATES=true 开启），支持修改清明节及春/秋假的起始日期。
...
export default async function (ctx) {
  ...
  const showExclusiveDates     = getBool("SHOW_EXCLUSIVE_DATES", false);
  ...
  const customDays = showExclusiveDates ? [1,2,3,4,5,6].map(i => ({
